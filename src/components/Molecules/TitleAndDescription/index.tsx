...
const TitleAndDescription = ({ title, description, link }: TitleAndDescriptionProps) => {
  const { t } = useTranslation();
  return (
    <div className={styles.hero__textContainer}>
      <h1 className={styles.hero__title}>{title}</h1>
      <p className={styles.hero__desc}>{description}</p>
      <Link href={link} className={styles.hero__actionLink} aria-label={t('components.heroActionLink.ariaLabel')}>
